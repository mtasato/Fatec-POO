...
export function BusModal({ isOpen, onClose, onSubmit, busToEdit }: BusModalProps) {
    const [model, setModel] = useState("");
    const [brand, setBrand] = useState("");
    const [color, setColor] = useState("");
    const [year, setYear] = useState(2024);
    const [licensePlate, setLicensePlate] = useState("");
    const [numberOfSeats, setNumberOfSeats] = useState(45);
    ...
    useEffect(() => {
        if (busToEdit) {
            ...
            setBrand(busToEdit.brand);
            setColor(busToEdit.color);
            setYear(busToEdit.year);
            setLicensePlate(busToEdit.licensePlate);
            setNumberOfSeats(busToEdit.numberOfSeats);
            setHasWifi(busToEdit.hasWifi);
            setHasAirConditioning(busToEdit.hasAirConditioning);
            setSelectedFeatures(busToEdit.accessibilityFeatures.map(f => f.id));
        } else {
            resetForm();
        }
    }, [busToEdit, isOpen]);
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4">
                <div className="p-6">
                    <h2 className="text-2xl font-title text-primary-color mb-6">
                        {busToEdit ? "Editar Ônibus" : "Cadastrar Ônibus"}
                    </h2>
                    ...
                    <form onSubmit={handleSubmit}>
                                        ...
                                        {features.map((f) => (
                                            <label key={f.id} className="flex items-center gap-2 cursor-pointer hover:bg-neutral-10 p-2 rounded">
                                                <input
                                                    ...
                                                    className="w-4 h-4"
                                                />
                                                <span className="text-sm text-neutral-70">{f.name}</span>
                                            </label>
                                        ))}
                                    </div>
                                )}
                        ...
                        <div className="flex gap-3">
                            ...
                            >
                                {busToEdit ? "Salvar Alterações" : "Cadastrar"}
