...
export default function ListVan() {
    ...
    const [sortField, setSortField] = useState<"brand" | "numberOfSeats">("brand");
    ...
    async function loadVans() {
        try {
            const response = await apiRequest(
                `/vans?page=${currentPage}&size=${ITEMS_PER_PAGE}&sort=${sortField},${sortOrder}`
            );
        ...
        <Layout>
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex gap-2 mb-4 items-center">
                    ...
                    <select
                        value={sortField}
                        onChange={(e) => setSortField(e.target.value as "brand" | "numberOfSeats")}
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b-2 border-neutral-20">
                                        <th className="text-left py-4 px-4 font-title text-title">Modelo</th>
                                        <th className="text-left py-4 px-4 font-title text-title">Marca</th>
                                        <th className="text-left py-4 px-4 font-title text-title">Assentos</th>
                                        <th className="text-left py-4 px-4 font-title text-title">Acessibilidades</th>
                                        <th className="text-left py-4 px-4 font-title text-title">Ações</th>
                                    </tr>
                                </thead>
